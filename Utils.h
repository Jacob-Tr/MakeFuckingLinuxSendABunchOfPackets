...
bool strInsert(char* dest, const char* src, const char ch, const size_t index, const size_t dest_size, const size_t src_len)
{
    ...
    if(index > src_len)
    {
        ...
        for(size_t i = src_len; i < index; i++) *(temp + i) = ' ';
    }
    else
    {
        temp = (char*) malloc((src_len + 1) * sizeof(char));
        snprintf(temp, (src_len + 1), "%s", src);
    }
    if(index != 0)
    {
        if(!substring(temp, src, 0, index, dest_size))
        {
            fprintf(stderr, "Error: strInsert failed substring call on index %zu-%zu in string \"%s\".\n", (size_t) 0, index, src);
    ...
    *(temp + index) = '\0';
    if(!substring((temp + (index + 1)), src, (index + 1), src_len, dest_size))
    {
        fprintf(stderr, "Error: strInsert failed substring call on index %zu-%zu in string \"%s\".\n", index, src_len, src);
...
static inline void fprintcmp(FILE* file, const char* str)
{
    ...
    for(size_t i = 0; i < length; i++)
    {
        ...
        if(c < (char) 9 && c != '\0')
        {
            if(!strInsert(temp, temp, '[', i, temp_size, (strlen(temp) + 1)))
            {
                fprintf(stderr, "Warning: fprintcmp failed call to strInsert adding \'%c\'[code: %d] to string \"%s\" at index %zu.\n", '[', '[', temp, i);
            }
            *(temp + (i + 1)) = *(temp + (i + 1)) + '0';
            if(!strInsert(temp, temp, ']', (i + 2), temp_size, (strlen(temp) + 1)))
            {
                fprintf(stderr, "Warning: fprintcmp failed call to strInsert adding \'%c\'[code: %d] to string \"%s\" at index %zu.\n", ']', ']', temp, i);
            }
...
int main() 
{
    ...
    compressText(str, str, 128, (strlen(str) + 1));
    
    fprintcmp(stdout, str);
    
    //fprintf(stdout, "%s | size: %zu", str, strlen(str));
    return 0;
}
